...
export const QuestDetailsViewPlayWrapper = observer(
  ({ selectedQuestId }: QuestDetailsViewPlayWrapperProps) => {
    ...
    return (
      <QuestDetails
        ...
        description={
          <MarkdownDescription
            classNames={{ root: styles.markdownDescription }}
          >
        ...
        eligibility={{
          reputation: {
            games: steamGames,
            completionPercent:
              questMeta.eligibility?.completion_threshold ?? 100,
            eligible: false,
            steamAccountLinked: false
          },
        ...
        isQuestsPage={true}
        onPlayClick={async () =>
          navigateToGame.mutateAsync(questMeta.project_id)
        }
        key={`questDetailsLoadedId${
          questMeta.id
        }streak${!!questPlayStreakData}`}
      />
    )
  }
)
