...
{
  std::cout << "ENTERED INJECT PROCESS test two" << std::endl;
  Napi::Env env = info.Env();
  ...
  result.Set("injectHelperExists", Napi::Value::From(env, inject_helper_exist));
  result.Set("injectDllExists", Napi::Value::From(env, inject_dll_exist));
